...
(re-frame/reg-event-fx
 ...
 (fn [cofx _]
   (let [persisted-db (read-string (:localStorage cofx))
         additional-tools (merge keer-db/default-db    ;; Add DB for Keer Tool
     ...
     {:db (if persisted-db
            (merge additional-tools db/default-db persisted-db)
            (merge additional-tools db/default-db))}))) ;; Home DB
